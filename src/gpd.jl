...
function gpdfit(x::AbstractVector, wip::Bool = true, min_grid_pts::Int = 30, sort_x::Bool = true)
    ...
    θHats = @. 1 / x[n] .+ (1 - sqrt((m+1) / seq)) / prior ./ quartile
    ξHats = mean(log1p.(- θHats .* x'), dims = 2)  # take mean f each row
    logLikelihood = @. n * (log(- θHats / ξHats) - ξHats - 1)  # Calculate log-likelihood at each estimate
